...
export function AppLayout({ children, activePage, userRole = "candidato", userName = "Lucía Ramírez" }: AppLayoutProps) {
  ...
  const candidatoLinks = [
    ...
    { name: "Mi Plan", path: "/PlanCapacitacion", icon: BookOpen },
    { name: "Capacitación", path: "/ModuloIA", icon: GraduationCap },
    { name: "Progreso", path: "/ProgresoyLogros", icon: LineChart },
    { name: "Mi Tutor", path: "/CanalTutor", icon: MessageSquare },
  ];

  const tutorLinks = [
    { name: "Mis Candidatos", path: "/PanelTutor", icon: Users },
    { name: "Alertas", path: "/PanelTutor", icon: AlertCircle },
    { name: "Reportes", path: "/PanelTutor", icon: FileText },
  ];
